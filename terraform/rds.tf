# Security Group para o RDS PostgreSQL do servico de Processamento
resource "aws_security_group" "processamento_db_sg" {
  name        = "${var.db_identifier}-sg"
  description = "Security group para RDS PostgreSQL Instance do servico de Processamento"
...
resource "aws_db_subnet_group" "processamento_subnet_group" {
...
# Instancia RDS PostgreSQL dedicada para o microsservico de Processamento
resource "aws_db_instance" "processamento_postgres" {
  ...
  db_subnet_group_name   = aws_db_subnet_group.processamento_subnet_group.name
  vpc_security_group_ids = [aws_security_group.processamento_db_sg.id]
